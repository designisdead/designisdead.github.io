...
<script>
  const loadData = function ({api, cacheVersion, errorCallback, version, path}) {
    return api.get(`cdn/stories/${path}`, {
      ...
      if (!res.response) {
        ...
        errorCallback({statusCode: res.response.status, message: res.response.statusText})
      }
    })
  }
   ...
   */
  const enrichJsonObject = async function ({elements, api, version, cacheVersion, errorCallback}) {
    let richObj = [];
   ...
   */
  const getListContent = async function ({elements, api, version, cacheVersion}) {
    return await api
      .get("cdn/stories", {
        version: version,
        cv: cacheVersion,
